...
async function main() {
  console.log("🧪 Testing Simple Q&A Contract Functionality...");
  ...
  const submitAnswerTx = await simpleQA.connect(user2).submitAnswer(
    2, // Question ID (assuming this is the second question)
  ...
  console.log("\n❓ Question Details:");
  const [qId, asker, title, description, bounty, isActive, hasApprovedAnswer, approvedAnswerId, qTimestamp] = await simpleQA.getQuestion(2);
